...
async function merge(context, pullRequest) {
  ...
  const mergeMethod = mergeUseMethodLabels
    ? getMergeMethod(defaultMergeMethod, mergeLabels, pullRequest)
    : defaultMergeMethod;
...
function skipPullRequest(context, pullRequest) {
  const {
    config: { mergeForks, mergeLabels, mergeUseMethodLabels }
  } = context;
  ...
  const allowedSuffixes = mergeUseMethodLabels
    ? ["", ".merge", ".rebase", ".squash"]
    : [""];
  ...
  for (const required of mergeLabels.required) {
    if (!allowedSuffixes.some(suffix => labels.includes(required + suffix))) {
...
function getMergeMethod(defaultMergeMethod, mergeLabels, pullRequest) {
  for (const l of pullRequest.labels) {
    for (const ml of mergeLabels.required) {
      switch (l.name) {
        case `${ml}.merge`:
          return "merge";
        case `${ml}.rebase`:
