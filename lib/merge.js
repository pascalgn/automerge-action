...
async function merge(context, pullRequest) {
  ...
  if (reviewMessage && reviewMessage.trim().length) {
    try {
      await pullRequestCreateReview(octokit, pullRequest, reviewMessage);
    } catch (e) {
      logger.info("Failed create review:", e.message);
    }
  }

  if (context.config.mergeMethod === "none") {
    return false;
  }
...
async function pullRequestCreateReview(octokit, pullRequest, reviewMessage) {
  logger.info("Creating pull request review with message:", reviewMessage);
