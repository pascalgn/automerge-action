...
const API_URL_REGEXP = /^https:\/\/api.github.com\/repos\/([^/]+)\/([^/]+)\/(pull)\/([^ ]+)$/;
...
async function executeFromJson(context, json) {
  executeGitHubAction(context,"check_suite",json)
}
...
async function handleCheckUpdate(context, eventName, event) {
  ...
  if (action !== "completed") {
    ...
    logger.info("payload conclusion", payload.conclusion)
    ...
    if (payload.conclusion === "success") {
      ...
      logger.debug("PullrequestData ",pullRequest)
...
module.exports = { executeLocally, executeGitHubAction ,executeFromJson };
